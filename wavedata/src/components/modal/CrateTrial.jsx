...
export default function CreateTrialModal({show, onHide}) {
	...
	async function CreateTrial(e) {
		...
		try {
			await window.sendTransaction(window.contract.CreateTrial(Number(Cookies.get("userid")),image.value,title.value,description.value, JSON.stringify(permissionData), 0,0,parseInt(budget.value)));
