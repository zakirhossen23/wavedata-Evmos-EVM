...
function TrialDetails() {
	...
	async function UpdateAudiences(event) {
		...
		await window.sendTransaction(contract.UpdateAudience(parseInt(params.id), JSON.stringify(createdObject)));
	...
	async function UpdateRewarads(event) {
		...
		try {
			await window.sendTransaction(contract.UpdateReward(Number(parseInt(params.id)), rewardselect.value, Number(rewardprice.value.replace("tEVMOS", "")), parseInt(totalspendlimit.value.replace("tEVMOS", ""))));
		} catch (error) { }
	...
	async function deleteTrial() {
		await window.sendTransaction(contract.delete_a_trial(Number(parseInt(params.id))));
